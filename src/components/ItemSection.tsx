...
> {
  try {
    let response: Response = await fetch(PUBLIC_SHEET_URL);
    let csvText: string = await response.text();
    console.log(csvText);
    let rows: string[][] = parseCSV(csvText);

    // Extract headers
    let headers: string[] = rows[0];

    // Convert rows to objects
    let objects: { [key: string]: string }[] = rows.slice(1).map((row) => {
